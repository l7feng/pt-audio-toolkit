...
| **设置** | **默认路径设置…** ← 7 个路径字段集中一处改，改完即存为默认 / 恢复出厂默认（仅路径） |
...
对话框只管 7 个**路径**字段（`input_dir` / `output_dir` / `log_dir` / `data_dir` /
`temp_dir` / `import_draft_dir` / `import_pkg_out`；v2.6.0 起新增日志/数据目录）。
「恢复出厂默认」**只重置路径**，不碰你调好的命名模板 / 规格 / 开关。

动作逻辑在 `core/menus.py`（纯标准库、可单测），对话框在 `tabs/paths_dialog.py`。

## 导出产物结构（v2.6.0）

输出目录下按产物类型分三大类，日志与运行数据独立在外（不混产物区）：

```
<输出目录>/
├── 01-多条WAV/<草稿名>/<集名>/      ← 整轨模式（等长对齐，导入 PT 即就位）
...
整轨命名支持 `{轨道类别}` 占位符（MX=音乐 / DX=对白 / SFX=音效 / AiFX 自动判定），
定案模板：`{项目名} EP{集数} {轨道类别} {序号:02d}` → `法老 EP14 DX 01.wav`。
完整规范见知识库「剪映导出产物命名规范模板」（41 号笔记）。
...
> 哪天搬家要在「设置 → 默认路径设置…」里改 7 个路径字段，而不是删目录。

## 已验证的剪映版本与版本防线（v2.7.0）

- **当前已验证**：剪映 **11.5.0**（草稿为 6.x 多时间线结构：`Timelines/<main_timeline_id>/draft_content.json` 为权威源）。
- **外部风险**：剪映官方无草稿格式承诺，升级后 `videoeditor.dll` / 草稿 JSON 结构一变，解析链路可能整条失效。
- 三道防线：
  1. 本节标注已验证版本 —— 升级剪映前先看这里；
  2. **启动时自动扫描草稿库**各草稿的 schema 版本，出现「没见过的版本」即在窗口顶部挂黄条告警（不硬崩；点黄条可查看分布并「记为已知」）；
  3. **工具 → 一键备份草稿库…**：robocopy 整库复制到草稿库旁边带时间戳的备份夹，升级剪映前建议先备份一次。
